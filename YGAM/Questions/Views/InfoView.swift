...
class InfoView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        // Arrow
        toolTipPath.move(to: topLeft(CGFloat((tooltipWidth / 2) - 8), 0))
        toolTipPath.addLine(to: topLeft(CGFloat(tooltipWidth / 2), -8))
        toolTipPath.addLine(to: topLeft(CGFloat(tooltipWidth / 2 + 8), 0))
        toolTipPath.close()
